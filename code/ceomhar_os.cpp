...
internal void OS_TakeEvent(OS_Event *event) {
...
internal b32 OS_PeekMouseMove(OS_Event **event) {
    // TODO(Cian):  not sure would there be more than one mouse event per frame? Might be possible
    b32 result = false;
    for(OS_Event *new_event = 0; OS_GetNextEvent(&new_event);) {
...
internal b32 OS_PeekMouseButtonEvent(OS_Event **event, OS_EventType type, OS_MouseButton button) {
    b32 result = false;
    // TODO(Cian):  not sure would there be more than one mouse event per frame? Might be possible
    for(OS_Event *new_event = 0; OS_GetNextEvent(&new_event);) {
...
internal OS_Event OS_MouseMoveEvent(V2 position, V2 delta_pos) {
...
internal OS_Event OS_MouseButtonDownEvent(OS_MouseButton button, V2 position) {
    OS_Event mouse_event = {};
...
internal OS_Event OS_MouseButtonUpEvent(OS_MouseButton button, V2 position) {
    OS_Event mouse_event = {};
